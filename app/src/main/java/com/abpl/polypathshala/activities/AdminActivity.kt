...
class AdminActivity : BaseActivity(), View.OnClickListener, NavigationView.OnNavigationItemSelectedListener {
...
fun setupNavigationView(){
    ...
    override fun onClick(p0: View?) {
        when (p0?.id) {
            R.id.userList -> {
                startNewActivityWithoutFinish(UserListActivity::class.java)
            }
            R.id.addSubject -> {
                startNewActivityWithoutFinish(AddSubject::class.java)
            }
    ...
    override fun onNavigationItemSelected(item: MenuItem): Boolean {
        ...
        when (item.itemId) {
            R.id.nav_item_one ->  startNewActivityWithoutFinish(UserListActivity::class.java)
            R.id.nav_item_two ->  startNewActivityWithoutFinish(AddSubject::class.java)
            R.id.nav_item_three ->  startNewActivityWithoutFinish(SubjectListActivity::class.java)
            R.id.nav_item_four -> { SignupService.instance?.userAuthRefrence?.signOut()
